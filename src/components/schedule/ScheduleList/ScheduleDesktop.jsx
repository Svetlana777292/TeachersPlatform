...
const ScheduleDesktop = ({days, setEditingLesson, isTeacher}) => {
    ...
    return (
        <main className="schedule-main schedule-desktop">
            <div className="calendar-header">
                <div className="time-corner" />
                {days.map((day, i) => (
                    <div className="day-header" key={i}>
                        <span className="day-name">{WEEK_DAYS[i]}</span>
                        <span className={days[i].toDateString() === currentDay.toDateString() ? "current-day" : "day-number"}>{days[i].getDate()}</span>
                    </div>
                ))}
            </div>
            ...
            <div className="calendar-body">
                <div className="time-column">
                    {HOURS.map(hour => (
                            <div className="time-slot" key={hour} style={{height: `${HOUR_HEIGHT}px`}}>
                ...
                {(days).map((day, i) => (
                    <div className="day-column" key={i}>
                        {HOURS.map(hour => (
                            <div className="hour-cell" key={hour} style={{height: `${HOUR_HEIGHT}px`}}/>
                        ))}
